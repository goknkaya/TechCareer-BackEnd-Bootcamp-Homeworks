...
namespace BusinessLayer
{
    public class TBusinessLayer
    {
        ...
        //ÖDEVDE EKLENDİ
        public List<TblDoctorInfo> GetDoctorInfo(out string OMessage)
        {
            List<TblDoctorInfo> Result = new List<TblDoctorInfo>();
            OMessage = "";
            try
            {
                Result = Context.Database.SqlQuery<TblDoctorInfo>("select * from TblDoctorInfo order by DoctorID desc").ToList();
                //Result = Context.Database.SqlQuery<TblDoctorInfo>("select * from TblDoctorInfo").ToList();
                //Result = (from Data in Context.TblDoctorInfo select Data).ToList(); //where eklenebilir
                //Result = (from DoctorInfo in Context.TblDoctorInfo orderby DoctorInfo.DoctorNameSurname ascending select DoctorInfo).ToList();
